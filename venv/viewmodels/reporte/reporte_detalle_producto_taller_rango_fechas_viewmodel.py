...
class ReporteDetalleProductoTallerRangoFechas(ViewModelBase):
    ...
    async def load(self, fecha_inicio, fecha_termino):
        ...
        if self.esta_conectado:
            self.registros = await reporte_service.get_reporte_detalle_producto_taller_rango_fechas(K_COD_REPORTE, self.ano_academ, self.fecha_inicio, self.fecha_termino, self.id_usuario_conectado)
        else:
